...
let participantes = [
    ...
    {
        nome: "Luiz Otávio",
        email: "luizotavio.03@gmail.com",
        dataInscricao: new Date(2024, 3, 1, 23, 30),
        dataCheckIn: new Date(2024, 1, 1)
    },
    {
...
const atualizarTabela = (participantes) => {
    let output = "";
...
const adicionarParticipante = (event) => {
    ...
    const participanteJaExiste = participantes.find((p) => p.email == participante.email
    )
...
const fazerCheckIn = (event) => {
    ...
    const mensagemConformacao = 'Tem certeza que deseja fazer o check-in?'

    if (confirm(mensagemConformacao) == false) {
        return
    }
